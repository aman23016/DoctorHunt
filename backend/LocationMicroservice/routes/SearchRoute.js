...
//       WHERE station_name = ?
//     `;
//     const [locationResults] = await db.promise().query(locationQuery, [locality]);
...
//     `;
//     const [doctorResults] = await db.promise().query(doctorQuery, [
...
// module.exports = router;  // Make sure to export the router so it can be used in server.js
// routes/SearchResultsRoute.js
const express = require('express');
...
router.post("/search", async (req, res) => {
  ...
  try {
    const doctors = await SearchService.searchDoctors(locality, doctorType, date);
    res.json({
      success: true,
      message: "Doctors found successfully",
      doctors,
